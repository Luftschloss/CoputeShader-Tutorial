...
设计目标是把确定的静态地形数据从运行时移到 Editor Bake，减少移动相机时的 GC 和 CPU spikes。`TerrainNode.cs` 这类运行时 node rebuild 路径已经不再是主链路。
...
运行时不维护对象树，children 用显式 index 访问。LOD traversal 只在数组里递归或索引，不需要创建 node 对象。
...
关键细节：`heightMinMax` 已经是 world-space y，不是 heightmap 0..1。GPU culling 不再采样高度图估算 bounds，而是直接使用 Editor Bake 生成的保守范围。
...
当前 normal texture 中的 RGB 实际是 `zyx` 排布，但 shader 没有再反 swizzle。后续验证视觉光照方向时需要重点检查该路径；目前基础光照已经依赖这张 normal。
...
该处理用于兼容部分 Unity normal map 导入后把 X 放到 alpha 的格式。当前 shader 还没有真正 blend layer normal，只是数据已经 Bake 进去。
...
1. `EnsureLodConfigDefaults()`，按 `bakedData.LodCount` 对齐 `lodConfigs`。
...
实现说明：`BindTerrainRenderProperties()` 当前每帧 `LateUpdate` 都会调用，并且会执行 `UpdateTerrainShaderArrays()`。当前策略用于保证材质和全局参数同步，后续如果继续优化 CPU，可考虑只在 baked data / LOD debug color / material debug mode 变化时刷新。
...
该阈值避免相机小幅移动导致每帧重建 LOD。例如 patchSize=64、lodCount=4 时，leafPatchSize=8，实际最小 rebuild 距离至少 4。
...
CollectActiveNodes(root)
  if node.mip <= 0 -> emit
  else if distance(cameraXZ, node.center) >= lodConfigs[node.mip].distance -> emit
  else if no children -> emit
  else recurse children
```

`lodConfigs[mip].distance` 越大，该 mip 越晚被使用。`GetLodDistance` 在越界或配置为空时返回 0，作为保守 fallback。
...
它同时决定 LOD 距离和 shader debug color。`EnsureLodConfigDefaults()` 会把数组长度对齐到 `bakedData.LodCount`；没有 baked data 时回退到 `DefaultTerrainLodConfigCount = 5`。新增默认项按 `0..DefaultMaxLodDistance` 线性分布，当前默认最大距离是 `1000`。
...
该 early-out 会跳过 active set 未变化时的 CPU/GPU 数据更新，减少相机移动但未跨 LOD rebuild boundary 时的开销。
...
Profiler 分段用于定位 Neighbors、Lookup、Upload 等阶段的开销，后续可针对高占比阶段继续优化。
...
实现细节：neighbor 查找没有每次递归搜索 quadtree，而是用 `TerrainLeafLookup`。
...
该查找覆盖多 terrain tile 边界的 LOD seam，同时要求 terrain tile 的 world rect 连续且 Bake 顺序稳定。
...
该判定和 `GpuDrivenHizMap.compute` 的 mip reduce 方向配套：

- reversed-Z：mip 保存 min depth。
- normal-Z：mip 保存 max depth。

关键细节：frustum 使用当前相机 VP，Hi-Z 使用生成 pyramid 时记录的 VP。该设计避免同一帧内 camera 或 matrix 不一致导致用错误坐标系采样 Hi-Z。
...
但当前 `GpuDrivenHizFeature` 没有搜索或调用 `GPUTerrain.DrawHiZDepth`。目前实际 Hi-Z pyramid 输入主要来自 URP camera depth target，而不是显式把 GPU terrain 重新画进 Hi-Z mip 0。
...
`argsBuffer` 是标准 5 uint indirect args：
...
patch mesh 本身只是一个归一化网格，真实地形几何完全由 baked node rect + heightmap displacement 生成。
...
设计约束：`DebugView` 只控制 SceneView wire，LOD Color 是 shader 颜色调试，不重新并入 DebugView 枚举。
...
- Debug 关闭时不做 `argsBuffer.GetData`、`hizStatsBuffer.GetData`、`visibleNodeInfoBuffer.GetData` 等同步读回。
...
实现说明：SceneView 能看到 GPUDriven 地形本身，是因为 `LateUpdate` 中 Editor 下 forward draw 使用：
...
该策略避免普通运行时因为 stats 显示引入 CPU-GPU sync。当前 GUI Stats 在 Debug 关闭时只显示轻量信息：mode、debug、color、CPU frame、Hi-Z on/off、terrain patches、foliage instances、status。
...
编辑侧保留 Unity Terrain、TerrainLayer、alphamap、Terrain Tools 等现有工作流。运行时只消费 baked mirror data，减少 runtime 复杂度。
...
`DepthTextureGenerator` 记录生成 Hi-Z 的 matrixVP 和 camera position。Compute culling 使用这套数据投影 bounds，避免同帧 camera 状态变化造成 Hi-Z uv-depth 不一致。
...
1. 编辑侧使用 Unity Terrain，运行侧使用 baked asset。
2. `GpuTerrainBakedData` 存储静态数据，重点是 flat quadtree 和 heightMinMax。
3. CPU LOD 从 baked root nodes 选择 active patches。
4. LOD rebuild 通过 persistent arrays、buffer capacity、active set early-out 降低移动时 GC。
5. LOD seam 通过 neighbor mask + quad vertex color snapping 处理。
6. Compute culling 基于 baked AABB 执行 frustum 和 Hi-Z。
7. Hi-Z pyramid 从 URP camera depth 生成，并记录生成时 VP 用于一致的遮挡投影。
8. Indirect draw 使用 visible id buffer + args buffer。
9. Shader 基于 patch rect + heightmap array 生成真实地形，并执行 TerrainLayer blend。
10. 当前边界包括第一阶段材质、terrain depth injection 未接 Feature、holes/PBR/RVT/SVT 未完成。
