...
本文档记录当前工程里的 `GPUDrivenTerrain` 实现状态，面向面试说明使用。旧版内容不保留；本文只覆盖当前代码实际存在的实现、非常规设计取舍、已知边界和后续可优化点。常规 GPU Driven 概念只在必要处点到为止。
...
面试时可以这么讲：当前 Hi-Z culling 框架和 terrain depth injection 函数都在，但真正稳妥的后续改进是把 `DrawHiZDepth` 接进 `GpuDrivenHizFeature` 的 mip0 构建阶段，明确让 terrain 作为 occluder 参与 Hi-Z。
